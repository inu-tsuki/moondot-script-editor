...
- ~~修复 textarea 自动高度与 focus 样式变化之间的裁切/跳动风险。~~ 已在 `7051593` 修复：所有 editor 移除 `focus:px-2 focus:py-1`，统一使用 `p-0`；新增 `adjustHeight()` + `onFocus` 高度重算。
- ~~给 Dialogue block 增加窄屏宽度策略。~~ 已在 `7051593` 修复：`max-w-full min-[860px]:max-w-[65%]`。
...
1. ~~Each auto-resizing textarea recalculates height only when `block.text` changes, but focus styles add padding/border and, for dialogue, change alignment.~~ Fixed in 7051593.
2. ~~Dialogue uses `max-w-[65%]` everywhere, too narrow on mobile.~~ Fixed in 7051593.
