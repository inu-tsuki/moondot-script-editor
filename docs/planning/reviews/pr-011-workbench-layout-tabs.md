...
- 生成大纲后切到 `outline` tab。
- YAML 下载/复制反馈切到 `yaml` tab，或提升反馈层级。
- 移除 outline list 的旧高度限制，或引入 compact/full mode。
...
1. After generating an outline from the topbar, switch to the `outline` tab. Otherwise users can click “大纲” while viewing YAML/Diagnostics and not see the new result.
2. `SceneOutlinePanel` still has `max-h-[220px]` from the old stacked output layout. In an isolated output tab, that artificially constrains the outline list.
