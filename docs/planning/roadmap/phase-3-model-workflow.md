...
> 最近更新：2026-06-07  
> 状态：Phase 3.2 已合并；当前推进 Phase 3.3 WriterBrief / scene draft contract。
...
- 建立 server-side schema registry / resolver，将 Phase 3.2 / 3.3 的 `schemaId` 映射到真实 SDK structured output 参数。
...
- refusal / empty output / parse / schema mismatch 不进入假成功 artifact，由 adapter 统一转成 `ModelCallError` 和 diagnostics。
