...
> 一个以导入和转换为入口的剧本编辑器：能处理 3 个以上章节的小说长文本，生成剧本 AST，提供结构化预览和语义块编辑，并把 AST 序列化为 YAML 剧本；同时附带 YAML Schema 文档解释设计原因。

不建议把第一版定位成完整现代剧本编辑器或完整 Story IDE。这个方向有长期价值，但会稀释官方题目的核心：小说转剧本、YAML 输出、Schema 文档。也不建议把第一版理解成“YAML 模板生成器”，因为 YAML 只是 AST 的提交表示。
...
- 官方要求“结构化剧本（YAML 格式）”。AST / Scene / Block 模型可以自然序列化为 YAML。
- 官方要求“可编辑、可进一步打磨”。brainstorm 明确指出不能让作者盯着 YAML 写作，需要编辑界面。
- 官方要求“Schema 文档说明设计原因”。brainstorm 已经形成 AST 核心模型和 YAML/Fountain/VN 等导出关系，可转化为 Schema 设计理由。
...
- 输出：展示 AST 驱动的结构化编辑视图，并导出符合 `script-yaml-schema.md` 的 YAML。
...
- 提供一个小型 AST inspection panel，证明 YAML 来自结构化模型而非文本拼接。
...
核心模型是 AST。YAML 是官方要求的序列化产物，`YAML + Fountain` 也是可选导出方案，不是核心存储格式。
...
- 每次合并后主分支保持可运行。
- demo 视频需要覆盖输入 3 个以上章节、生成结构化剧本 AST、编辑/预览、导出 YAML、Schema 文档位置。
