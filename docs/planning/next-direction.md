...
> 最近更新：2026-06-07
> 状态：Phase 3.4 server-side wire-up 已完成；当前准备 Phase 3.4b frontend proxy adapter。
...
- Phase 3.4 正式阶段 (server-side wire-up)：`/api/model/call` endpoint 完整实现，打通 `ModelCallRequest → OpenAI Responses API → structured output → parseAndNormalizeProviderOutput → app-side Zod safeParse → ModelCallResult`。8 种 `ModelCallError` 映射覆盖 config_missing / network / refusal / empty_output / parse / schema。Handler 和 16 个测试已在 `src/server/handler.ts` 和 `tests/server/handler.test.ts`。
