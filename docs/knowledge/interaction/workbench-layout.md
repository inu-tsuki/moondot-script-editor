...
可以借鉴 `playground/kmd` 的 dock / workbench 思路，但不建议直接搬代码。
...
如果后续需要更强布局能力，优先考虑在 React 中实现轻量 resizable panels 或引入专门的 React panel library，而不是把 KMD 的 dock 实现整块迁移。
...
如果引入，优先比较 Tailwind 与 UnoCSS：

- Tailwind：生态稳定，评审和合作者更容易理解。
- UnoCSS：更轻、更灵活，但配置表达更偏工程化。

在当前项目节奏下，Tailwind 比 UnoCSS 更适合作为未来候选；但第一版继续使用普通 CSS。
