...
> 最近更新：2026-06-06  
> 状态：Phase 2.5.5b 基础切片已落地。本文记录中央 reading surface 的语义编辑器交互契约和后续扩展边界。

## 核心目标

2.5.5 让中央编辑区像剧本稿。2.5.5b 进一步让它能完成一轮真实编辑：改场景、改对白、增删块、调整顺序、修正角色和括号提示，并持续更新 YAML / diagnostics。
...
2.5.6 已将 toolbar 收口为右侧小气泡形态。它锚在稿纸 gutter 中，而不是覆盖正文的任意浮层。

具体约束：

- 默认放在 selected block 的右侧，不放在左侧正文起点。左侧更容易打断 action / narration 的阅读节奏。
- `ManuscriptSurface` 应为工具条预留右侧 gutter；工具条可以视觉上 floating，但布局上不能遮挡 block text。
- 主工具条约 32px 高，按钮 28x28px，使用 icon-only button 和 `aria-label` / `title`。
- 主工具条只放 Move up、Move down、Add after、Delete。Add after 打开类型菜单，菜单列出 Action / Dialogue / Narration / Transition / Note。
- Add after 菜单可以临时覆盖稿纸边缘，但不能挡住当前 textarea 光标；窄屏下应改为 block 下方 inline menu。
- 不先引入 Floating UI。当前场景只需要在已知容器内定位，CSS + React state 足够。若后续出现 viewport 自动翻转、portal layering、复杂嵌套菜单或 resizable panel 碰撞，再评估 `@floating-ui/react`。
...
这些函数让 UI 通过明确 action 修改 `ScreenplayDocument`，避免在组件里手写深层 document mutation。
