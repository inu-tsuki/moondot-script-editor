...
> 最近更新：2026-06-06
> 状态：MVP 交互方向已落地为中央手稿式语义块编辑器，后续仍以 `ScreenplayDocument` 为主输入模型。
...
- 查看章节列表和章节摘要。
- 查看 scene outline 和当前剧本场景。
- 编辑场景标题、摘要、地点、内外景和时间。
- 编辑 action、dialogue、narration、transition 和 note 文本。
- 按类型增加语义块。
- 删除语义块。
- 调整语义块顺序。
- 将 dialogue 绑定到已有角色。
- 编辑或清空 dialogue parenthetical。

仍可后续补强：

- 编辑项目标题和 target medium 等 document metadata。
- 新增角色并用于对白。
- 多场景 sidebar 或 scene 切换编辑。
- 局部 AI 重写、版本历史和 source coverage map。
...
Phase 2.5.5 已把中心编辑区升级为由 `ScreenplayDocument` 支撑的 Fountain-like reading surface。它不是让用户直接编辑 Fountain 文本，而是在 document metadata、角色表、source refs 和语义块上覆盖更像剧本稿的排版语言。
...
完整基础编辑流程见 `screenplay-editor-ux.md`。阅读面解决“像剧本稿”；基础语义编辑控件解决“能完成一轮打磨”。后续更高级的角色管理、局部 AI 重写和版本历史仍不属于当前 MVP 基础切片。

## 推荐界面结构

```text
Top: project / generation / export actions
Left: Source / Chapters / Preferences
Center: 月点 Semantic Manuscript Editor
Right: Scene Outline / YAML / Diagnostics tabs
```

中心区域是主工作区：

- Scene heading / title / synopsis。
- Manuscript block list。
- Block toolbar 和 add block menu。
- Dialogue character selector 和 parenthetical input。
