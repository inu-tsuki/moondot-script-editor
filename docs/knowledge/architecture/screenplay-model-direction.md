...
> 状态：初步收敛，供 MVP 设计、AST 契约和 YAML 序列化 Schema 使用。

## 核心判断

本项目不能只做“小说文本进、YAML 文本出”的转换器。官方要求产物是“可编辑、可进一步打磨的剧本初稿”，因此核心应是可编辑的剧本 AST / 领域模型。YAML 必须服务于 AST 的提交、存储和校验，而不是成为作者需要手写维护的负担。

当前方向是：

- 内部用结构化 AST / 项目模型表示剧本。
- 对外优先把 AST 序列化为官方要求的 YAML。
- UI 提供可读、可编辑的剧本视图。
...
- AST 驱动的结构化预览/编辑区：让作者看到章节、场景、角色、对白和动作。
- YAML 导出区：把 AST 序列化为官方提交格式。
- Schema 文档：解释 AST 的 YAML 投影字段为什么这样设计。
...
这个结构比直接 Fountain 更适合官方题目，因为官方明确要求 YAML Schema。它也比只用纯文本更适合 AI 生成，因为模型可以按 AST 字段生成、校验和局部修复。
...
- `YAML`：主交付格式，完整表达剧本初稿。
- `Schema 文档`：解释 AST 的 YAML 序列化字段、层级和设计原因。

MVP 可选加分：

- `Fountain-like preview`：在 UI 中模拟影视剧本阅读格式。
- `Ren'Py/Naninovel stub`：展示一个小片段如何从 AST/YAML 编译到 VN 脚本。
- `YAML + Fountain export`：展示 AST 可以被导出为混合文本，但不作为核心存储。
...
- 因此第一版应做“可编辑的 AI 改编工作台”：导入章节、生成剧本 AST、可视化编辑、序列化导出 YAML。
